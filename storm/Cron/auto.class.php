  ...
  array (
    ...
    'native' => true,
		...
		public static function INSERT() {
			$db = Database::getInstance();
			$sql = "INSERT INTO `Cron` (`id`, `__timestamp__`, `__operation__`) VALUES (NULL, ".time().", 'INSERT')";
			$result = $db->sql($sql);
			$id = mysql_insert_id();
			return self::ROW($id);
		}
		...
		public function DELETE($physical=true) {
			$db = Database::getInstance();
			if ($physical) {
				$sql = "DELETE FROM `Cron` WHERE id='".$this->id."'";
				unset(self::$data[$this->id]);
			} else {
				$sql = "UPDATE `Cron` SET `__timestamp__` = ".time().", `__operation__` = 'DELETE' WHERE `id`='".$this->id."'";
			}
			$db->sql($sql);
		}
		...
		// Setters and Getters
public function setText($value) { $this->row['Text'] = $value; $value = mysql_real_escape_string($value); $timestamp = time(); $sql = "UPDATE `Cron` SET `Text`='$value',`__timestamp__` = $timestamp, `__operation__` = 'UPDATE' WHERE `id`='{$this->id}'"; Database::getInstance()->sql($sql);} public function getText() { return $this->row['Text']; }
